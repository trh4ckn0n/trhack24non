...
def choose_country(countries):
    ...
    while True:
        country_input = questionary.text("\U0001F30D Entrez un pays (ex: France, Spain, Italy)").ask()
...
def track_flight(fr, flight, open_browser=False):
    ...
    try:
        ...
        console.print("\n[bold red]Tracking interrompu par l'utilisateur.[/bold red]")
...
if __name__ == "__main__":
    ...
    if airport:
        ...
        if flights:
            selected_flight = questionary.select("🛬 Choisissez un vol à suivre :", choices=flights).ask()
            if selected_flight:
                open_browser = questionary.confirm("Ouvrir le vol dans le navigateur ?").ask()
